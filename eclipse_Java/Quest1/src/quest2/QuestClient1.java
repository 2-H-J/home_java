...
public class QuestClient1 {
    ...
    public static void main(String[] args) {
        ...
        final int SERVER_PORT = 9999;// 123456 -> 9999

        try (
                Socket server = new Socket(SERVER_IP, SERVER_PORT);
                Scanner sc = new Scanner(System.in);
                PrintWriter pw = new PrintWriter(server.getOutputStream(), true);
                BufferedReader br = new BufferedReader(new InputStreamReader(server.getInputStream()));//new InputStreamReader 추가
        ) {
            System.out.print("숫자를 입력하세요: ");
            int facNumber = sc.nextInt();

            pw.println(facNumber); // 추가
